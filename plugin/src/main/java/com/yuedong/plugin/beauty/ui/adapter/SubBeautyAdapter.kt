 ...
 * 单选有返回的Adapter
 */
class SubBeautyAdapter(private val level1Data: MakeupData) :
    RecyclerView.Adapter<SubBeautyAdapter.ViewHolder>() {
    ...
    private val listener = View.OnClickListener {
        ...
        } else if (position != level1Data.nowSelect) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        when (data.bgType()) {
            bg_type_icon -> holder.icon.setPadding(
                10.toPX().toInt(), 10.toPX().toInt(), 10.toPX().toInt(), 10.toPX().toInt()
            )
        ...
        holder.iconBorder.visibility =
            if (level1Data.nowSelect == position) View.VISIBLE else View.INVISIBLE
        var glideBuilder: RequestBuilder<Drawable>? = null
        when (holder.iconBorder.isSelected) {
            true -> {
                if (data.srcSelectedPath != "") glideBuilder =
                    Glide.with(holder.itemView).load(File("$rootDir/${data.srcSelectedPath}"))
                        .centerInside()
            }

            else -> {
                if (data.srcNotSelectedPath != "") glideBuilder =
                    Glide.with(holder.itemView).load(File("$rootDir/${data.srcNotSelectedPath}"))
                        .centerInside()
            }
        }
        when (data.shape()) {
